...
from .core import create_asymmetric_window, compute_fft, calibrate_spectrum, compute_hsi_for_voltage  # Import compute_fft and calibrate_spectrum
...
def create_unified_widget(hsi_dict, wavelengths, positions_dict, base_dir):
    ...
    def autoscale(b):
        ...
        if ref_vgs != 'None':
            ref_img = hsi_dict[vds][ref_vgs][wavelength_idx, :, :]
        ...
        spectrum = hsi[:, y, x]
        if ref_vgs != 'None':
            ref_spectrum = hsi_dict[vds][ref_vgs][:, y, x]
